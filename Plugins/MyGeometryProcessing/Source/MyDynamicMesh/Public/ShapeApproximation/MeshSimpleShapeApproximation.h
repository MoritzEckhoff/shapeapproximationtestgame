 ...
 *
 */
class MYDYNAMICMESH_API FMeshSimpleShapeApproximation
{
public:
	...
	/**
	* Fit containing axis-aligned box, oriented box, capsule, and sphere to each input mesh, and
	* store the one with lowest cost in ShapeSetOut.
	* Constraints for the fitted meshes are:
	* 1. The intersection of source mesh and fitted mesh needs to be equal to the source mesh
	* 2. The fitted volume needs to be equal to or greater than the source volume
	*/
	void Generate_MinCostApproximation(FSimpleShapeSet3d& ShapeSetOut);
