...
struct TConstrainedDelaunay2
{
	 ...
	 */
	bool MYGEOMETRYALGORITHMS_API Triangulate();
	 ...
	 * @return false if Triangulation failed
	 */
	bool MYGEOMETRYALGORITHMS_API Triangulate(TFunctionRef<bool(const TArray<TVector2<RealType>>&, const FIndex3i&)> KeepTriangle);
};
 ...
 */
template<typename RealType>
TArray<FIndex3i> MYGEOMETRYALGORITHMS_API ConstrainedDelaunayTriangulate(const TGeneralPolygon2<RealType>& GeneralPolygon);
 ...
 */
template<typename RealType>
TArray<FIndex3i> MYGEOMETRYALGORITHMS_API ConstrainedDelaunayTriangulateWithVertices(const TGeneralPolygon2<RealType>& GeneralPolygon, TArray<TVector2<RealType>>& OutVertices);
